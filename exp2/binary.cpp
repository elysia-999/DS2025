...
int main() {
    ...
    string speech = 
        ...
        "we will be able to speed up that day when all of gods children black men and white men jews and gentiles protestants and catholics will be able to join hands and sing in the words of the old negro spiritual free at last free at last thank god almighty we are free at last";
        
    cout << "=== Huffman Coding System ===" << endl;
    cout << "Original Text Length: " << speech.length() << " characters" << endl;
    
    // 创建Huffman编码器
    HuffmanEncoder encoder(speech);
    
    cout << "\n=== Frequency Table ===" << endl;
    encoder.displayFrequencyTable();
    
    cout << "\n=== Huffman Code Table ===" << endl;
    encoder.displayCodeTable();
    
    // 编码单词
    cout << "\n=== Word Encoding ===" << endl;
    vector<string> words = {"dream", "freedom", "justice", "brotherhood", "equality"};
    
    for (const string& word : words) {
        string encoded = encoder.encodeWord(word);
        cout << word << " -> " << encoded << " (Length: " << encoded.length() << " bits)" << endl;
    }
    
    // 编码整个文本
    cout << "\n=== Text Encoding ===" << endl;
    Bitmap* encodedBitmap = encoder.encodeText();
    char* bitString = encodedBitmap->bits2string(encodedBitmap->size());
    
    cout << "Encoded text (first 200 bits): " << string(bitString).substr(0, 200) << "..." << endl;
    cout << "Total encoded bits: " << encodedBitmap->size() << endl;
    
    // 计算压缩率
    int originalBits = speech.length() * 8; // 假设原始是ASCII
    double compressionRatio = (1.0 - (double)encodedBitmap->size() / originalBits) * 100;
